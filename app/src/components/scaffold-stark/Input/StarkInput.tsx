...
import { CommonInputProps, InputBase, SIGNED_NUMBER_REGEX } from '../../scaffold-stark'
import { useGlobalState } from '../../../services/store/store'

const MAX_DECIMALS_USD = 2

function starkValueToDisplayValue(usdMode: boolean, starkValue: string, currencyPrice: number) {
...
function displayValueTostarkValue(usdMode: boolean, displayValue: string, currencyPrice: number) {
    if (usdMode && currencyPrice) {
...
export const StarkInput = ({
    ...
    const displayValue = useMemo(() => {
        const newDisplayValue = starkValueToDisplayValue(internalUsdMode, value, currencyPrice)
        if (
    ...
    const handleChangeNumber = (newValue: string) => {
        ...
        const newStrkValue = displayValueTostarkValue(internalUsdMode, newValue, currencyPrice)
